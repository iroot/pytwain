...
class MainWindow(Frame, TwainBase):
    def __init__(self, title):
        ...
        File = Menu(menubar)
        File.add_command(label="Connect", command=self.MnuOpenScanner)
        File.add_command(label="Acquire Natively", command=self.MnuAcquireNatively)
        File.add_command(label="Acquire By File", command=self.MnuAcquireByFile)
        File.add('separator')
        File.add_command(label="Exit", command=self.MnuQuit)
